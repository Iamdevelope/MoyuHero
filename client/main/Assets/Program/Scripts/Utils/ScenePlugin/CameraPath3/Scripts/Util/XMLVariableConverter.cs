...
public class XMLVariableConverter
{
    ...
    public static AnimationCurve FromXMLtoAnimationCurve(XMLNode node)
    {
        ...
        foreach (XMLNode keyframeNode in node.GetNodeList("keyframe"))
        {
            Keyframe keyFrame = new Keyframe();
            keyFrame.inTangent = float.Parse(keyframeNode.GetValue("inTangent>0>_text"));
            keyFrame.outTangent = float.Parse(keyframeNode.GetValue("outTangent>0>_text"));
            keyFrame.time = float.Parse(keyframeNode.GetValue("time>0>_text"));
            keyFrame.value = float.Parse(keyframeNode.GetValue("value>0>_text"));
            output.AddKey(keyFrame);
        }
        return output;
    }
