...
public class RealTimeRoleColumn {
...
//	public static final int FIRSTLADDERINFO_ID = 1;//firstladderinfo表的唯一ID
	public static RealTimeRoleColumn getRealTimeRole(long roleId, boolean readonly)
	{
/*		if(xtable.Properties.select(roleId) == null)
			throw new IllegalArgumentException("构造RealTimeRole时，角色 "+roleId+" 不存在。");
		
		xbean.RealTimeRole xrole = null;
		if(readonly)
			xrole = xtable.Realtimeroles.select(roleId);
		else
			xrole = xtable.Realtimeroles.get(roleId);
		if(xrole == null)
		{
			if(readonly)
				xrole = xbean.Pod.newRealTimeRoleData();
			else
			{
				xrole = xbean.Pod.newRealTimeRole();
				xtable.Realtimeroles.insert(roleId, xrole);
			}
		}*/
		return null;
	}
...
//	private final xbean.RealTimeRole xrole;
	public RealTimeRoleColumn(long roleId, boolean readonly) {
		this.roleId = roleId;
...
//			rankers.add(rid);
	...
	private Map<Integer,Long> getEnermyRankers()
	{
		...
		{
			 RealTimeRoleColumn r = RealTimeRoleColumn.getLadderRole(enermyid, true);
	...
	{
		SLevelConfig cfg = ConfigManager.getInstance().getConf(SLevelConfig.class).get(xtable.Properties.get(roleId).getLevel());
...
//			return false;
		...
		if(iswin)
		{
			...
			{//仇敌
				...
				RealTimeRoleColumn targetlrole = RealTimeRoleColumn.getLadderRole(rankerId, false);
				if(targetlrole != null)
	...
	*/
	...
	public boolean setRank(int dstrank)
	{
		...
		if(dstladder != null)
		{//替换
			RealTimeRoleColumn dstrole = getLadderRole(dstladder.getRoleid(),false);
			if(srcladder != null)
